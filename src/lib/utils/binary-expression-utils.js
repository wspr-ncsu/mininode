...
function isDynamicBinaryExpression(node) {
  if (node.type === syntax.BinaryExpression) {
    if (node.left.type !== syntax.BinaryExpression) {
      return node.left.type !== syntax.Literal || node.right.type !== syntax.Literal;
    }
    let copy = node;
    while (copy.type === syntax.BinaryExpression) {
      if (copy.right.type !== syntax.Literal) {
        return true;
      } else if (copy.right.type === syntax.Literal && copy.left.type === syntax.Literal) {
        return false;
      }
      copy = copy.left;
    }
...
function getValue(node) {
  ...
  let left = node.left.type === syntax.Literal ? node.left.value : getValue(node.left);
  return left + right;
}
...
function getValues(node, identifiers) {
  let result = new Set(), rights = new Set(), lefts = new Set();
  if ((node.left.type !== syntax.Literal && node.left.type !== syntax.Identifier && node.left.type !== syntax.BinaryExpression)
    || (node.right.type !== syntax.Literal && node.right.type !== syntax.Identifier)) {
      return null;
    }
  
  if (node.right.type === syntax.Literal) {
    if (typeof node.right.value === 'string' || typeof node.right.value === 'number')
  ...
  if (node.left.type === syntax.Literal) {
    if (typeof node.left.value === 'string' || typeof node.right.value === 'number')
...
function getIdentifiers(node) {
  let result = new Set();
  let rtype = node.right.type, ltype = node.left.type;
  if (rtype !== syntax.Literal && rtype !== syntax.Identifier) return null;
  if (ltype !== syntax.Literal && ltype !== syntax.Identifier && ltype !== syntax.BinaryExpression) return null;
  ...
  while (copy.type === syntax.BinaryExpression) {
    ...
    } else if (copy.left.type !== syntax.Literal && copy.left.type !== syntax.BinaryExpression) {
      return null;
    }
    copy = copy.left;
  }
