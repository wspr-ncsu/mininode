...
function minimizeModule(modul) {
  ...
  if (config.compressLog) {
    ...
    for (let key of Object.keys(modul)) {
      ...
      if (Array.isArray(modul[key]) && modul[key].length === 0)
        delete modul[key];
    }
  } else {
  ...
  try {
    ...
    if (!modul.parseError) {
      ...
      if (!(modul.path.includes("/.bin/") || modul.path.includes("/bin/"))) {
        ...
        console.debug(
          `[utils.js] Deleting a file from the disk "${modul.path}"`
        );
