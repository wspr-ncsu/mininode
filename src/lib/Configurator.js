...
const argv = require("yargs")
  ...
  .option("seeds", {
    type: "array",
    describe:
      "Seed files from where to start building the dependency graph. By default mininode will try to resolve the entry point of the application by reading the package.json file",
  })
  .option("dry-run", {
    describe:
      "Generates the mininode.json (dependency graph) file without reducing the application",
    type: "boolean",
  })
  .option("silent", {
    describe:
      "Runs in silent mode, i.e. does not print anything to the console",
    type: "boolean",
  })
  .option("verbose", {
...
if (argv.defaultAttackSurface)
  settings.default_attack_surface = argv.defaultAttackSurface;
if (argv.mode) {
