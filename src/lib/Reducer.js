    ...
    enter: function (node, parent) {
      ...
      switch (node.type) {
        case es.Syntax.Identifier:
          ...
          if (parent 
          && (parent.type !== es.Syntax.VariableDeclarator || (parent.init && parent.init.type === es.Syntax.Identifier && parent.init.name === node.name)) 
          ...
          && parent.type !== es.Syntax.ObjectPattern) {
            ...
            let varElement = variables.filter(e => e.scope <= currentScope && e.id === node.name).pop();
            if (varElement) {
              varElement.node['xUsed'] = true;
            }
            
            let funcElement = functions.filter(e => e.scope <= currentScope && e.id === node.name).pop();
            if (funcElement) {
        ...
        case es.Syntax.VariableDeclarator:
          if (node.id.type === es.Syntax.Identifier) {
            ...
            idIndex = idstr.findIndex(e => (e.scope >= currentScope && e.id === node.id.name));
            if (idIndex > -1) {
              node['xUsed'] = true;
            } else {
              node['xUsed'] = false;
            ...
            for (let prop of node.id.properties) {
              ...
              idIndex = idstr.findIndex(e => (e.scope >= currentScope && e.id === prop.key.name));
              if (idIndex > -1) {
                prop['xUsed'] = true;
              } else {
                prop['xUsed'] = false;
        ...
        case es.Syntax.FunctionDeclaration:
          ...
          if (!node.xUsed) {
            idIndex = idstr.findIndex(e => (e.id === node.id.name && e.scope >= currentScope - 1));
            if (idIndex > -1) {
              node['xUsed'] = true;
            } else {
              node['xUsed'] = false;
        ...
        case es.Syntax.CallExpression:
          let funcElement = functions.filter(e => (e.id === node.callee.name && e.scope <= currentScope)).pop();
          if (funcElement) {
            funcElement.node['xUsed'] = true;
          }
          break;
        case es.Syntax.MemberExpression:
          if (parent && parent.type === es.Syntax.AssignmentExpression && parent.left === node) {
            let meta = helper.getMemberExpressionMeta(node);
            if (meta && !meta.computed && (meta.exported || modul.exporters.includes(meta.object))) {
              let full = meta.object + '.' + meta.property;
              let prop = meta.exported ? helper.getExportedProperty(full) : meta.property;
              if (prop) {
                if (modul.used.includes(prop) || modul.selfUsed.includes(prop) || extra.includes(prop) || full === 'module.exports') {
                  if(config.verbose) console.log('-- used:', chalk.blue(full));
                  parent['xUsed'] = true;
                  if (full === 'module.exports' && parent.right && parent.right.type === es.Syntax.ObjectExpression) {
                    for (let property of parent.right.properties) {
                      let key = property.key;
                      let keyName = key.type === es.Syntax.Identifier ? key.name : key.type === es.Syntax.Literal ? key.value : '';
                      if (modul.used.includes(keyName) || modul.selfUsed.includes(keyName) || extra.includes(keyName)) {
                        property.xUsed = true;
                      } else {
                  ...
                  if(config.verbose) console.log('-- not used:', chalk.yellow(full));
                  parent['xUsed'] = false;
          ...
          else if (parent && parent.type === es.Syntax.CallExpression && node.object.name === 'Object' && node.property.name === 'defineProperty') {
            let arg1 = parent.arguments[0];
            let arg2 = parent.arguments[1];
            if (arg2.type === es.Syntax.Literal) {
              if ((arg1.type === es.Syntax.Identifier && arg1.name === 'exports') 
                  || (arg1.type === es.Syntax.MemberExpression && arg1.object.name === 'module' && arg1.property.name === 'exports')) 
              {
                let prop = arg2.value;
                if (modul.used.includes(prop) || modul.selfUsed.includes(prop) || extra.includes(prop)) {
                  parent.xUsed = true;
                } else {
                  parent.xUsed = false;
                }
